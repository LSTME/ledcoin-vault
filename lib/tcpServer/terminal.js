...
module.exports = class Terminal {
  ...
  auth() {
    ...
    return this.tell('GetAuth', args).then((res) => {
      ...
      if (!proto.IsValid(callerId, responseObject.targetId, targetChallenge, responseObject.targetSignature)) {
        this.info('Signature from wallet is invalid', undefined, 'warning');
        return false;
      }
  ...
  syncWalletBalance(walletId) {
    ...
    return this.auth()
      .then((authResponse) => {
        ...
        const value = this.dataSource.getTransactionsForWallet(authResponse.targetId)
          .reduce((acc, t) => acc + Number(t.deltaCoin), 0);
  ...
  info(message, prefix, level = 'info') {
    this.logger[level](`[${prefix ? `${prefix}-` : ''}${this.key}] ${message}`);
  }
