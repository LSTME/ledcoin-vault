...
module.exports = class Terminal {
  ...
  async readLog(type) {
    ...
    let ptr = 0;
    const startId = logBuffer.readUInt16BE(ptr); ptr += 2;
    const length = logBuffer.readUInt16BE(ptr); ptr += 2;
    const records = [];
    for (let i = 0; i < length; i += 1) {
      const record = {};
      if (type === 0) {
        record.targetId = logBuffer.readUInt16BE(ptr); ptr += 2;
      } else if (type === 1) {
        record.targetId = logBuffer.readUInt8(ptr); ptr += 1;
        const recData = logBuffer.readUInt8(ptr); ptr += 1;
        record.gameId = recData >> 6 && 0x03;
        record.playsCount = recData >> 4 && 0x03;
        record.coins = recData && 0x0F;
      }
      records.push(record);
    }
    return {
